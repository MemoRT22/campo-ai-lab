...
export class DebugPanel {
  ...
  update(now: number): void {
    ...
    const lines = [
      ...
      `manos       ${status.hands.state}${status.hands.segmentation ? ' + seg' : ''} · ${status.hands.fps.toFixed(1)} fps · ${status.hands.inferenceMs.toFixed(1)} ms inf · age ${handAge} ms · ${field.handsApplied}/${status.hands.hands} vistas${status.hands.tooFar > 0 ? ` · ${status.hands.tooFar} lejos` : ''}${status.hands.lastError ? ` · ${summarize(status.hands.lastError)}` : ''}`,
